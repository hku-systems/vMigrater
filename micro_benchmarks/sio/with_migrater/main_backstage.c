...
void *thread_func(void *arg) {
	...
	while(1) {
		...
		if (diff_time > DIFF_USEC) {
			...
			vcpu[vn].timeslice = vcpu[vn].end_time - vcpu[vn].start_time;
			vcpu[vn].counter = 0;
			...
			//if (vcpu[io_vn].left_time <= 4000) {
				...
				//counter_affi += 1;
			//}
			if ((sm->flag == 1) && (-1 != get_pid_affinity(sm->pid))) {
		...
		//}
#if 1
		if((vcpu[vn].left_time <= (int64_t) 4000) && (vn == get_pid_affinity(sm->pid))) {
			io_vn = get_max_left_time();
			if ((io_vn != vn) && (vcpu[io_vn].left_time >= (int64_t) 6000)) {
				set_pid_affinity(io_vn, sm->pid);
			}
#if 1
			if (vcpu[vn].left_time <= (int64_t) 1500) {
				if (vcpu[io_vn].left_time >= 2000) {
					set_pid_affinity(io_vn, sm->pid);
				} else {
					io_vn = predict_one_vcpu();
					set_pid_affinity(io_vn, sm->pid);
