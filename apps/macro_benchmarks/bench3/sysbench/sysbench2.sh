...
echo "sysbench, macrobench 3 is on vCPU $1"
	...
	#cd /home/kvm1/sda3
/usr/bin/sysbench --test=fileio --file-total-size=10G prepare > /dev/null
echo "============> sysbench start to run===============>"
/usr/bin/taskset -c $1 /usr/bin/sysbench --test=fileio --max-time=30 --max-requests=0 --file-total-size=8GB --num-threads=$2 --file-test-mode=seqrd run
echo "============> sysbench end===============>"
/usr/bin/sysbench --test=fileio --file-total-size=10G cleanup > /dev/null
